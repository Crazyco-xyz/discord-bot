...
class OnJoinListener(commands.Cog):
    ...
    async def remove_user_if_not_completed_captcha(self, member: nextcord.Member):
        ...
        logger.info(f"Waiting for {config.guild_captcha_timeout} minutes to see if user {member.name} has solved the captcha")
        ...
        for role in member.roles:
            if role.id == captcha_role.id:
                logger.info(f"User {member.name} still has the captcha role after {config.guild_captcha_timeout} minutes. Kicking...")
                try:
